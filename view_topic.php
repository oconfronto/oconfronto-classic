...
include("templates/private_header.php");
	...
	$foruminfo = $db->execute("select * from `forum_question` where `id`=?", array($_GET['id']));
...
if ($_GET['up'])
{
	...
	$jaupou = $db->execute("select * from `thumb` where `topic_id`=? and `player_id`=?", array($_GET['id'], $player->id));
	...
	$upar1 = $db->execute("update `forum_question` set `up`=`up`+1 where `id`=?", array($_GET['id']));
	echo "Obrigado por votar! <a href=\"view_topic.php?id=" . $_GET['id'] . "\">Voltar</a>.";
	include("templates/private_footer.php");
	exit;
}elseif ($_GET['down']) {

	$jadown = $db->execute("select * from `thumb` where `topic_id`=? and `player_id`=?", array($_GET['id'], $player->id));
	...
	$down1 = $db->execute("update `forum_question` set `down`=`down`+1 where `id`=?", array($_GET['id']));
	echo "Obrigado por votar! <a href=\"view_topic.php?id=" . $_GET['id'] . "\">Voltar</a>.";
	include("templates/private_footer.php");
	exit;
}
...
echo "<b><font size=\"1\"><a href=\"select_forum.php\">Fóruns</a> -> <a href=\"main_forum.php?cat=" . $rows['category'] . "\">" . ucfirst($categoria) . "</a> -> <a href=\"view_topic.php?id=" . $rows['id'] . "\">" . ucfirst($rows['topic']) . "</a></font></b>";
...
<table width="560px" bgcolor="#f2e1ce">
  <tr>
    <td width="110px" bgcolor="#E1CBA4"><center><img src="<? echo $topicouser['avatar']; ?>" width="100px" height="100px" border="0"></center><br/><center><font size="1"><b><a href="profile.php?id=<? echo $topicouser['username']; ?>"><?php if($topicouser['gm_rank'] > 2){  echo "<font color=\"green\">" . $topicouser['username'] . "</font>"; } elseif ($player->serv != $topicouser['serv']){ echo "<font color=\"red\">" . $topicouser['username'] . "</font>"; }else{ echo $topicouser['username']; } ?></a></b><br/><b>Posts:</b> <? echo $topicouser['posts']; ?>
<br/><?php
...
<BR><BR>
<?php
$conta = $db->execute("select `a_id` from `forum_answer` where `question_id`=?", array($_GET['id']));
...
$query = $db->execute("select `id`, `username`, `avatar`, `posts`, `ban`, `alerts`, `gm_rank`, `serv` from `players` where `id`=?", array($rows['a_user_id']));
$user = $query->fetchrow();

?>
<table width="560" bgcolor="#f2e1ce">
  <tr>
    <td width="110" bgcolor="#E1CBA4"><center><img src="<? echo $user['avatar']; ?>" width="100px" height="100px" border="0"></center><br/><center><font size="1"><b><a href="profile.php?id=<? echo $user['username']; ?>"><?php if($user['gm_rank'] > 2){  echo "<font color=\"green\">" . $user['username'] . "</font>"; } elseif ($player->serv != $user['serv']){ echo "<font color=\"red\">" . $user['username'] . "</font>"; }else{ echo $user['username']; } ?></a></b><br/><b>Posts:</b> <? echo $user['posts']; ?>
<br/><?php
...
}else if ($player->gm_rank > 2){
echo "<a href=\"edit_answer.php?topic=" . $rows['question_id'] . "&a=" . $rows['a_id'] . "\">Editar</a> | <a href=\"delete_answer.php?topic=" . $rows['question_id'] . "&a=" . $rows['a_id'] . "\">Deletar</a><br>";
}
?><?php
$respoxtak = stripslashes($rows['a_answer']);
...
$viewcount = $db->execute("update `forum_question` set `view`=`view`+1 where `id`=?", array($id));
...
<tr>
<td><input name="id" type="hidden" value="<? echo $id; ?>"><script>edToolbar('a_answer'); </script><textarea name="a_answer" rows="6" id="a_answer" class="ed"></textarea></td>
</tr>
<tr>
