	...
	case "attack":
		...
		if ($selectbixo->recordcount() == 0) {
			...
			if (!$_GET['id']){
				...
				$mhpp = $db->execute("select `hp` from `monsters` where `id`=?", array($_GET['id'] / $player->id));
					if ($mhpp->recordcount() < 1) {
						header("Location: monster.php");
						break;
					}else{
						$surprisequest1 = rand (1, 500);
						$surprisequest2 = rand (1, 900);
...
echo "</fieldset>";
	...
	$checamagiastatus = $db->execute("select * from `magias` where `magia_id`=5 and `player_id`=?", array($player->id));
			...
			if ($player->hp > 0)
			{
				...
				else
				{
					...
					if (($randatacktype == 1) or ($player->hp < 100)){
						...
						}elseif (($specialdefesadupla == 1) and ($monsterdamage > ($playerdamage / 1.5)) and ($defesatriplaexists->recordcount() > 0) and ($player->mana >= 60) and ($fastturno == 0)){
							include("battle/fastbattle/defesaquinta.php");

						}elseif (($specialdefesadupla == 1) and ($monsterdamage > ($playerdamage / 2)) and ($defesaquintaexists->recordcount() > 0) and ($player->mana >= 25) and ($fastturno == 0)){
						...
						if (($specialmistichit == 1) and ($monsterdamage > ($playerdamage / 1.3)) and ($ataqueescudomistico->recordcount() > 0) and ($player->mana >= 75) and ($fastturno == 0)){
							include("battle/fastbattle/escudo.php");

						}elseif (($specialdoublehit == 1) and ($monsterdamage > ($playerdamage / 1.5)) and ($ataquequintaexists->recordcount() > 0) and ($player->mana >= 65)){
							include("battle/fastbattle/quintohit.php");

						}elseif (($specialdoublehit == 1) and ($monsterdamage > ($playerdamage / 2)) and ($ataquequintaexists->recordcount() > 0) and ($player->mana >= 30)){
			...
			if ($bixo->hp > 0){
				...
				if (($misschance <= $enemy->miss) or (($fastmagia == 6) and ($fastturno > 0)))
				{
					...
					$damage = rand($enemy->mindmg, $enemy->maxdmg); //Calculate random damage
						...
						$bixo->hp -= $damage;
						$_SESSION['ataques'] .= "<font color=\"green\">" . ucfirst($enemy->prepo) . " " . $enemy->username . " tentou te atacar mas seu ataque voltou e ele perdeu " . $damage . " de vida.</font><br/>";
						}else{
						$player->hp -= $damage;
						$_SESSION['ataques'] .= ($player->username == $enemy->username)?"<font color=\"green\">":"<font color=\"red\">";
						$_SESSION['ataques'] .= ucfirst($enemy->prepo) . " " . $enemy->username . " te atacou e você perdeu " . $damage . " de vida.<br/>";
						$_SESSION['ataques'] .= "</font>";
						}
		...
		$query = $db->execute("update `players` set `gold`=`gold`+?, `hp`=?, `mana`=?, `energy`=`energy`-10, `monsterkill`=`monsterkill`+1 where `id`=?", array($goldwin, $player->hp, $player->mana, $player->id));
			...
			$db->execute("update `players` set `mana`=?, `maxmana`=? where `id`=?", array($ganha, $ganha, $player->id));
				...
				$query = $db->execute("update `players` set `stat_points`=`stat_points`+3, `level`=`level`+1, `maxexp`=?, `hp`=`maxhp`+30, `maxhp`=`maxhp`+30, `exp`=?, `magic_points`=`magic_points`+1, `energy`=`energy`-10, `gold`=?, `monsterkill`=`monsterkill`+1, `monsterkilled`=`monsterkilled`+1 where `id`=?", array($expofnewlvl, $newexp, $player->gold + $goldwin, $player->id));
			}
			else
			{
				//Update player
				$query = $db->execute("update `players` set `exp`=`exp`+?, `gold`=`gold`+?, `hp`=?, `mana`=?, `energy`=`energy`-10, `monsterkill`=`monsterkill`+1, `monsterkilled`=`monsterkilled`+1 where `id`=?", array($expdomonstro, $goldwin, $player->hp, $player->mana, $player->id));
			}
		}
		...
		if (($player->hp < 1) or ($morreu == 5)){
			...
			$query = $db->execute("update `players` set `energy`=?, `exp`=?, `gold`=?, `deaths`=?, `hp`=0, `mana`=0, `deadtime`=? where `id`=?", array($player->energy - 10, $player->exp - $exploss3, $player->gold - $goldloss2, $player->deaths + 1, time() + $setting->dead_time, $player->id));
			$query = $db->execute("delete from `bixos` where `player_id`=?", array($player->id));
