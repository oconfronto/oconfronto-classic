...
switch($_GET['act'])
{
	case "attack":
		...
		if (($tolevelttyy < $enemy->level) and ($enemy->evento == 'f'))
		{
			include("templates/private_header.php");
...
echo "<br />";
$query = $db->execute("select items.id, items.item_id, blueprint_items.type, blueprint_items.name, blueprint_items.description, blueprint_items.img from `items`, `blueprint_items` where blueprint_items.id=items.item_id and items.player_id=? and blueprint_items.type='potion' and blueprint_items.id=137 order by blueprint_items.name asc", array($player->id));
...
else
{
	while($item = $query->fetchrow())
	{
		...
		$query = $db->query("select blueprint_items.effectiveness, blueprint_items.name, items.item_bonus, items.optimized from `items`, `blueprint_items` where blueprint_items.id=items.item_id and items.player_id=? and blueprint_items.type='weapon' and items.status='equipped'", array($player->id));
		$player->atkbonus = ($query->recordcount() == 1)?$query->fetchrow():0;
		$query = $db->query("select blueprint_items.effectiveness, blueprint_items.name, items.item_bonus, items.optimized from `items`, `blueprint_items` where blueprint_items.id=items.item_id and items.player_id=? and blueprint_items.type='armor' and items.status='equipped'", array($player->id));
		$player->defbonus = ($query->recordcount() == 1)?$query->fetchrow():0;
		$query = $db->query("select blueprint_items.effectiveness, blueprint_items.name, items.item_bonus, items.optimized from `items`, `blueprint_items` where blueprint_items.id=items.item_id and items.player_id=? and blueprint_items.type='helmet' and items.status='equipped'", array($player->id));
		$player->defbonus2 = ($query->recordcount() == 1)?$query->fetchrow():0;
		$query = $db->query("select blueprint_items.effectiveness, blueprint_items.name, items.item_bonus, items.optimized from `items`, `blueprint_items` where blueprint_items.id=items.item_id and items.player_id=? and blueprint_items.type='legs' and items.status='equipped'", array($player->id));
		$player->defbonus3 = ($query->recordcount() == 1)?$query->fetchrow():0;
		$query = $db->query("select blueprint_items.effectiveness, blueprint_items.name, items.item_bonus, items.optimized from `items`, `blueprint_items` where blueprint_items.id=items.item_id and items.player_id=? and blueprint_items.type='boots' and items.status='equipped'", array($player->id));
		$player->defbonus4 = ($query->recordcount() == 1)?$query->fetchrow():0;
		$query = $db->query("select blueprint_items.effectiveness, blueprint_items.name, items.item_bonus, items.optimized from `items`, `blueprint_items` where blueprint_items.id=items.item_id and items.player_id=? and blueprint_items.type='shield' and items.status='equipped'", array($player->id));
		$player->defbonus5 = ($query->recordcount() == 1)?$query->fetchrow():0;
		$query = $db->query("select blueprint_items.effectiveness, blueprint_items.name, items.item_bonus, items.optimized from `items`, `blueprint_items` where blueprint_items.id=items.item_id and items.player_id=? and blueprint_items.type='amulet' and items.status='equipped'", array($player->id));
		...
		else if ($player->voc == 'archer' and $player->promoted == 't')
		{
		$multipleatk = 0.90;
		$multipledef = 1.39;
		$divideres = 1.2;
		}
		else if ($player->voc == 'knight' and $player->promoted == 't')
		{
		$multipleatk = 0.99;
		$multipledef = 1;
		$divideres = 1.25;
		}
		else if ($player->voc == 'mage' and $player->promoted == 't')
		{
		$multipleatk = 1.44;
		$multipledef = 0.84;
		$divideres = 1.3;
		}
		else if ($player->voc == 'archer' and $player->promoted == 'r')
		{
		$multipleatk = 1;
		$multipledef = 1.48;
		$divideres = 1.15;
		}
		else if ($player->voc == 'knight' and $player->promoted == 'r')
		{
		$multipleatk = 1.08;
		$multipledef = 1.09;
		$divideres = 1.20;
		}
		else if ($player->voc == 'mage' and $player->promoted == 'r')
		{
		$multipleatk = 1.53;
		$multipledef = 0.90;
		$divideres = 1.25;
		}
		else if ($player->voc == 'archer' and $player->promoted == 's')
		{
		$multipleatk = 1.09;
		$multipledef = 1.56;
		$divideres = 1;
		}
		else if ($player->voc == 'knight' and $player->promoted == 's')
		{
		$multipleatk = 1.15;
		$multipledef = 1.16;
		$divideres = 1.1;
		}
		else if ($player->voc == 'mage' and $player->promoted == 's')
		{
		$multipleatk = 1.63;
		$multipledef = 1;
		$divideres = 1.15;
...
if ($enemy->loot > 1)
	...
	$sorteioitem = rand(1, 32);
	if ($sorteioitem == 32){
	$sorteiaitem = $db->execute("select `id`, `name` from `blueprint_items` where `type`!=? and `price`>? and `price`<? order by rand() limit 1", array(addon, $enemy->mtexp * 2.5, $enemy->mtexp * 3.5));
	...
	$lootstatus = 2;
		...
		if ($player->hp <= 0)
		{
			...
			$query = $db->execute("update `players` set `energy`=?, `exp`=?, `gold`=?, `deaths`=?, `hp`=0, `deadtime`=? where `id`=?", array($player->energy - 1, $player->exp - $exploss3, $player->gold - $goldloss2, $player->deaths + 1, time() + $setting->dead_time, $player->id));
		...
		else if ($enemy->hp <= 0)
		{
			...
			if ($enemy->mtexp + $player->exp >= $player->maxexp) //Player gained a level!
			{
				...
				$query = $db->execute("update `players` set `stat_points`=?, `level`=?, `maxexp`=?, `maxhp`=?, `exp`=?, `hp`=?, `energy`=?, `gold`=?, `monsterkill`=?, `monsterkilled`=? where `id`=?", array($player->stat_points + 3, $player->level + 1, $expofnewlvl, $player->maxhp + 30, $newexp, $player->maxhp + 30, $player->energy - 1, $player->gold + $goldwin, $player->monsterkill + 1, $player->monsterkilled + 1, $player->id));
			}
			else
			{
				//Update player
				$query = $db->execute("update `players` set `exp`=?, `gold`=?, `hp`=?, `energy`=?, `monsterkill`=?, `monsterkilled`=? where `id`=?", array($player->exp + $enemy->mtexp, $player->gold + $goldwin, $player->hp, $player->energy - 1, $player->monsterkill + 1, $player->monsterkilled + 1, $player->id));
			}
		...
		else
		{
			...
			$query = $db->execute("update `players` set `hp`=?, `energy`=?, `monsterkill`=? where `id`=?", array($player->hp, $player->energy - 1, $player->monsterkill + 1, $player->id));
...
$showitenx = $db->execute("select items.id, items.item_id, blueprint_items.name, blueprint_items.description, blueprint_items.img from `items`, `blueprint_items` where blueprint_items.id=items.item_id and items.player_id=? and blueprint_items.type='potion' order by rand() limit 7", array($player->id));
...
$tolevel=round($player->level * 1.8);
		...
		$sql = mysql_query("SELECT * FROM monsters WHERE level>='$fromlevel' AND level<='$tolevel' AND evento='f' order by level asc") or die(mysql_error());		
		if (mysql_num_rows($sql) > 0)//Check if any monsters were found
		{
		  ...
		  $bool = 1;
			...
			$sql2 = mysql_query("SELECT * FROM monsters WHERE evento='t' order by level asc") or die(mysql_error());
