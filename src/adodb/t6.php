...
$rs = $db->Execute($sql1);
echo "ADOdb no param: ";
var_dump($rs->GetArray());
$rs = $db->Execute($sql2, 1);
echo "ADOdb with param: ";
var_dump($rs->GetArray());

$my = new mysqli($host, $user, $password, $database);
echo "mysqli execute_query no param: ";
var_dump($my->execute_query($sql1)->fetch_assoc());
echo "mysqli execute_query with param: ";
var_dump($my->execute_query($sql2, [1])->fetch_assoc());
echo "mysqli query no param: ";
var_dump($my->query($sql1)->fetch_assoc());

$st = $my->prepare($sql2);
$st->execute([1]);
echo "mysqli execute prepared: ";
var_dump($st->get_result()->fetch_assoc());
