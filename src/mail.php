...
if ($_POST['sendmail'])
{
	...
	$ignorado = $db->execute("select * from `ignored` where `uid`=? and `bname`=?", array($sendto['id'], $player->username));
...
switch($_GET['act'])
{
	...
	$quereya = $db->execute("select * from `ignored` where `bname`=? and `uid`=?", array($_GET['add'], $player->id));
...
if($_GET['delete']){
$query = $db->execute("delete from `ignored` where `uid`=? and `bname`=?", array($player->id, $_GET['delete']));
if($query){
...
$query = $db->execute("select `bname` from `ignored` WHERE `uid`=?", array($player->id));
if ($query->recordcount() == 0)
{
...
break;
	...
	case "read": //Reading a message
		...
		if ($query->recordcount() == 1)
		{
			...
			$to = $db->GetOne("select `username` from `players` where `id`=?", array($msg['to']));
	...
	case "enviadas":
		...
		if ($query->recordcount() > 0)
		{
			...
			while($msg = $query->fetchrow())
			{
				...
				$to = $db->GetOne("select `username` from `players` where `id`=?", array($msg['to']));
				$servto = $db->GetOne("select `serv` to `players` where `id`=?", array($msg['to']));
				$rankto = $db->GetOne("select `gm_rank` to `players` where `id`=?", array($msg['to']));
				echo "<td width=\"20%\">";
				if ($rankto > 2){
	...
	default: //Show inbox
		...
		if ($query->recordcount() > 0)
		{
			...
			while($msg = $query->fetchrow())
			{
				...
				$from = $db->GetOne("select `username` from `players` where `id`=?", array($msg['from']));
				$servfrom = $db->GetOne("select `serv` from `players` where `id`=?", array($msg['from']));
				$rankfrom = $db->GetOne("select `gm_rank` from `players` where `id`=?", array($msg['from']));
				echo "<td width=\"20%\">";
