...
switch($_GET['act']) {
    case "attack":
        ...
        $query0 = $db->execute("select * from `bixos` where `player_id`=?", [$player->id]);
        if ($query0->recordcount() == 0) {
            ...
            if (!$_GET['id']) {
                ...
                $mhpp = $db->execute("select `hp` from `monsters` where `id`=?", [$uncodedid]);
                if ($mhpp->recordcount() < 1) {
                    ...
                    $query = $db->execute("update `players` set `lutando`=? where `id`=?", [$uncodedid, $player->id]);
                    ...
                    if ($player->hp > 0) {
                        ...
                        if ($misschance <= $player->miss) {
                            ...
                            if ($randatacktype == 1 || $player->hp < 100) {
                                if ($player->hp < 75 && $healexists->recordcount() > 0 && $player->mana >= 15) {
                                ...
                                } elseif ($specialdefesadupla == 1 && $monsterdamage > $playerdamage && $defesatriplaexists->recordcount() > 0 && $player->mana >= 60 && $fastturno === 0) {
                                    include(__DIR__ . "/battle/fastbattle/defesaquinta.php");

                                } elseif ($specialdefesadupla == 1 && $monsterdamage > $playerdamage && $defesaquintaexists->recordcount() > 0 && $player->mana >= 25 && $fastturno === 0) {
                                    ...
                                    $_SESSION['ataques'] .= "Voc� atacou " . $enemy->prepo . " " . $enemy->username . " e tirou " . $playerdamage . " de vida.<br/>";
                                    $_SESSION['ataques'] .= "</font>";
                                }
                            } elseif ($randatacktype == 2 || $randatacktype == 3) {
                                if ($specialmistichit == 1 && $monsterdamage > $playerdamage && $ataqueescudomistico->recordcount() > 0 && $player->mana >= 75 && $fastturno === 0) {
                                    include(__DIR__ . "/battle/fastbattle/escudo.php");

                                } elseif ($specialdoublehit == 1 && $monsterdamage > $playerdamage && $ataquequintaexists->recordcount() > 0 && $player->mana >= 65) {
                                    include(__DIR__ . "/battle/fastbattle/quintohit.php");

                                } elseif ($specialdoublehit == 1 && $monsterdamage > $playerdamage && $ataquequintaexists->recordcount() > 0 && $player->mana >= 30) {
                                    include(__DIR__ . "/battle/fastbattle/triplohit.php");
                                } else {
                                    $bixo->hp -= $playerdamage;
                                    $_SESSION['ataques'] .= ($player->username == $enemy->username) ? "<font color=\"red\">" : "<font color=\"green\">";
                                    $_SESSION['ataques'] .= "Voc� atacou " . $enemy->prepo . " " . $enemy->username . " e tirou " . $damage . " de vida.<br/>";
                                    $_SESSION['ataques'] .= "</font>";
                                }
                            }
                    ...
                    if ($bixo->hp > 0) {
                        ...
                        if ($misschance <= $enemy->miss || $fastmagia == 6 && $fastturno > 0) {
                            ...
                            if ($fastmagia == 10 && $fastturno > 0) {
                                ...
                                $_SESSION['ataques'] .= ucfirst((string) $enemy->prepo) . " " . $enemy->username . " te atacou e voc� perdeu " . $damage . " de vida.<br/>";
                                $_SESSION['ataques'] .= "</font>";
                            }
        ...
        if ($bixo->hp < 1 || $matou == 5) {
            ...
            if ($expfull == 1) {
                ...
                $query = $db->execute("update `players` set `gold`=`gold`+?, `hp`=?, `mana`=?, `energy`=`energy`-10, `monsterkill`=`monsterkill`+1, `lutando`=0 where `id`=?", [$goldwin, $player->hp, $player->mana, $player->id]);
                ...
                $query = $db->execute("update `players` set `stat_points`=`stat_points`+3, `level`=`level`+1, `maxexp`=?, `hp`=`maxhp`+30, `maxhp`=`maxhp`+30, `exp`=?, `magic_points`=`magic_points`+1, `energy`=`energy`-10, `gold`=?, `monsterkill`=`monsterkill`+1, `monsterkilled`=`monsterkilled`+1, `lutando`=0 where `id`=?", [$expofnewlvl, $newexp, $player->gold + $goldwin, $player->id]);
            } else {
                //Update player
                $query = $db->execute("update `players` set `exp`=`exp`+?, `gold`=`gold`+?, `hp`=?, `mana`=?, `energy`=`energy`-10, `monsterkill`=`monsterkill`+1, `monsterkilled`=`monsterkilled`+1, `lutando`=0 where `id`=?", [$expdomonstro, $goldwin, $player->hp, $player->mana, $player->id]);
            }
            ...
            if ($enemy->username == \ZEUS) {
                $medalha10 = $db->execute("select * from `medalhas` where `player_id`=? and `medalha`=?", [$player->id, 'Lend�rio']);
        ...
        if ($player->hp < 1 || $morreu == 5) {
            ...
            $query = $db->execute("update `players` set `energy`=?, `exp`=?, `gold`=?, `deaths`=?, `hp`=0, `mana`=0, `deadtime`=?, `lutando`=0 where `id`=?", [$player->energy - 10, $player->exp - $exploss3, $player->gold - $goldloss2, $player->deaths + 1, time() + $setting->dead_time, $player->id]);
            $query = $db->execute("delete from `bixos` where `player_id`=?", [$player->id]);
    ...
    default:
        ...
        ($sql = mysqli_query($db, "SELECT * FROM monsters WHERE level>='$fromlevel' AND level<='$tolevel' AND evento!='n' order by level asc")) || die(mysqli_error($db)());
