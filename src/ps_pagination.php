...
class PS_Pagination {
 ...
 public $page = 1;
	...
	function __construct(public $sql, $rows_per_page = 10, $links_per_page = 5, public $append = "") {
  ...
  */
 function renderFirst(string $tag = 'Primeira'): bool|string {
		if ($this->total_rows == 0) {
      return FALSE;
  }
  ...
  */
 function renderLast(string $tag = 'Ùltima'): bool|string {
		if ($this->total_rows == 0) {
      return FALSE;
  }
  ...
  */
 function renderNext(string $tag = '&gt;&gt;'): bool|string {
		if ($this->total_rows == 0) {
      return FALSE;
  }
  ...
  * @param string $tag Text string to be displayed as the link. Defaults to '<<'
  */
 function renderPrev(string $tag = '&lt;&lt;'): bool|string {
		if ($this->total_rows == 0) {
      return FALSE;
  }
  ...
  * @access public
  */
 function renderNav(string $prefix = '<span class="page_link">', string $suffix = '</span>'): bool|string {
		if ($this->total_rows == 0) {
      return FALSE;
  }
