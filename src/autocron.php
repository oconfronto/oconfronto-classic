...
$cura = $db->execute("update `players` set `hp`=`maxhp`, `deadtime`=0 where `hp`<1 and `deadtime`<?", array(time()));

$tempoip = ceil(time() - 1800);
$deletaip = $db->execute("delete from `ip` where `time`<?", array($tempoip));

$tempolog = ceil(time() - 604800);
$db->execute("delete from `mail` where `time`<?", array($tempolog));
$db->execute("delete from `user_log` where `time`<?", array($tempolog));
$db->execute("delete from `log_battle` where `time`<?", array($tempolog));
$db->execute("delete from `logbat` where `time`<?", array($tempolog));
$db->execute("delete from `revenge` where `time`<?", array($tempolog));
$db->execute("delete from `work` where `started`<?", array($tempolog));
...
{
    $updateWorkStatus = $db->execute("update `work` set `status`='f' where `id`=?", array($newwork['id']));
    if(!$updateWorkStatus){
        die("Error updating work status: " . $db->ErrorMsg());
    }

    $updatePlayers = $db->execute("update `players` set `gold`=`gold`+?, `energy`=`energy`/? where `id`=?", array(($newwork['gold'] * $newwork['worktime']), $newwork['worktime'], $newwork['player_id']));
    if(!$updatePlayers){
