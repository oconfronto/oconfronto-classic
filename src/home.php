...
<table width="100%" border="0">
	<tr>
		<td width="50%">
			...
			$data = date("j F, Y, H:i", $player->registered);
			$data_em_portugues = str_replace(array_keys($meses), array_values($meses), $data);
			?>
			<b>Usu�rio:</b> <?= $player->username ?><br />
			<b>Registrado:</b> <?= $data_em_portugues ?><br />
			<?php
			...
			if ($player->level > 144 and $player->level < 156 and $chdrgdrg->recordcount() == 0) { ?>
				<div style="width:90%; background-color:#EEA2A2; padding:5px; border: 1px solid #DEDEDE; margin-bottom:10px"
					align="center">
					<p>Voc� est� entre o nivel 145 e 155, e tem uma miss�o � fazer. <a href="quest5.php">Clique aqui</a>.
					</p>
				</div>
			<?php } ?>
		...
		<td width="50%">
			...
			?><br />
			<b>Vitalidade:</b> <?= $player->vitality ?>
			<?php
			echo " <font color=\"green\">+" . $vitalidadeeeeebonus . "</font>";
			?><br />
			<b>Agilidade:</b> <?= $player->agility ?>
			<?php
			...
			?><br />
			<b>Resist�ncia:</b> <?= $player->resistance ?>
			<?php
...
<b>Link para convidar amigos:</b> <a
	href="http://www.oconfronto.co.nr/?r=<?= $player->id ?>">http://www.oconfronto.co.nr/?r=<?= $player->id ?></a><br />
